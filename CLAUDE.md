...
Codex のモデルは `mcp__codex__codex` の `model` パラメータで切り替えられる。Claude は**タスクの難易度・内容に応じて最適なモデルを選択**する。

- 選択の目安（グレードで考える。具体的な model id は Codex 更新で変わるため、下段の参照先で都度確認する）:

  | タスクの性質                                         | 選ぶグレード                                            |
  | ---------------------------------------------------- | ------------------------------------------------------- |
  | 重い/複雑・設計判断や正しさが微妙・複数ファイル横断  | 最上位フロンティア（必要なら reasoning `high`/`xhigh`） |
  | 日常的・中規模・単一コンポーネントの機能追加         | バランス型                                              |
  | 軽微・定型・量産・機械的（typo/import/小さなCSS 等） | 高速・低コスト型                                        |

- 運用ルール:
  - **委譲時に「選んだモデル＋理由」を明示**する（透明性のため）。
  - ユーザーはいつでも上書き可（「これは高速型で」等）。
  - 迷ったら**1段上**のモデルを選ぶ（品質優先）。
- **現行の具体的な model id と一覧取得方法**は、揮発するためここには固定しない。memory の `reference_codex_models`、または `codex app-server` の JSON-RPC `model/list` で確認する。
- なお `model` 未指定時は `~/.codex/config.toml` の既定が使われる（＝方針を効かせるには毎回 `model` を明示する）。
