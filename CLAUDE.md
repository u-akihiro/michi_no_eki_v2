...
5. **実装前チェック（重要）**: Codex に「実装に着手する前に、以下3点をまず報告する」ことを明示的に要求する:
   - この仕様で不明・曖昧な点はあるか
   - 実装で失敗しうる落とし穴を最低3つ挙げる（依存衝突、CI/CD差異、将来の破壊的変更、セキュリティ、パフォーマンス、タイムゾーン、OS依存 等）
   - 仕様には無いが導入を推奨する改善点はあるか

   Claude はその報告を受けて、必要なら Codex の指摘に対応した仕様修正 or ユーザー確認を経てから実装を許可する。
